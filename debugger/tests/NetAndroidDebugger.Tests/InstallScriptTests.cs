...
{
    private static string ScriptPath => Path.Combine(TestEnvironment.RepoRoot, "install-vscode-extension.cmd");
    private static string RegisterPath => Path.Combine(TestEnvironment.RepoRoot, "register-mcp.cmd");
    ...
    /// same install folder register-mcp.cmd publishes to. Two scripts, one convention.
    /// </summary>
    [Fact]
    ...
    public void TheExtension_ContributesTheDocumentedDebugType()
    {
        var manifest = File.ReadAllText(Path.Combine(
            TestEnvironment.RepoRoot, "DevTools", "vscode", "net-android-debugger", "package.json"));
