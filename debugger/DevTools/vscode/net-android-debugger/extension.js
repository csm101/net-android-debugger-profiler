...
/**
 * Where `register-mcp.cmd` publishes by default. The adapter is a plain dotnet dll, so the whole
    ...
    const factory = {
        createDebugAdapterDescriptor() {
            ...
            if (!fs.existsSync(adapter)) {
                ...
                // bug report: the usual cause is simply that register-mcp.cmd has not been run.
                throw new Error(
                    `The debug adapter was not found at ${adapter}. Run register-mcp.cmd in the ` +
                    `net-android-debugger repository, or set "netAndroidDebugger.adapterPath".`);
            }
