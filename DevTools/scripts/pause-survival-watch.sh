...
# Results of the reference application run: ANDROID_ATTACH_NOTES.md, the reference application section (U6).
S=emulator-5554; PKG=App.Droid; OUT=u6_watch.log
echo "== U6 watch start $(date +%H:%M:%S)" > $OUT
adb -s $S logcat -c
for i in $(seq 1 9); do
  sleep 30
  PIDS=$(adb -s $S shell "ps -A | grep -i $PKG" | awk '{print $2":"$NF}' | tr '\n' ' ')
