...
rem
rem   vscode\install-vscode-extension.cmd   install into %USERPROFILE%\.vscode\extensions
rem   set NAD_VSCODE_EXTENSIONS=...    install elsewhere (VS Code Insiders, a portable install)
rem
rem Run register-mcp-debugger.cmd first: it publishes the adapter this extension launches. Re-running this
rem script is safe; it replaces whatever is already installed under that name.

for %%I in ("%~dp0..") do set "REPO=%%~fI\"
set "SOURCE=%REPO%vscode\net-android-debugger"
...
if exist "%DAP_DLL%" (
    ...
    echo     The extension will install, but a debug session fails until you run register-mcp-debugger.cmd.
)
...
if exist "%USERPROFILE%\.vscode-insiders\extensions" if "%NAD_VSCODE_EXTENSIONS%"=="%USERPROFILE%\.vscode\extensions" (
    ...
    echo     set NAD_VSCODE_EXTENSIONS=%%USERPROFILE%%\.vscode-insiders\extensions ^&^& vscode\install-vscode-extension.cmd
)
echo.
...
echo.
echo Every argument: vscode\DAP_CLIENTS.md
exit /b 0
