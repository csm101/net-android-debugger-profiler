...
3. Read the living specifications relevant to the next step:
   - collection/adb/msbuild/providers work -> `ANDROID_PROFILING_NOTES.md`
   - engine, session, SQLite schema, frontends -> `ARCHITECTURE.md`
...
6. Do not restart repository exploration from zero unless the recorded state
   is clearly stale or inconsistent.

If one or both state files are missing:

1. Say that a normal resume is required because persisted state is
   unavailable.
