...
## Current task
**Full MCP test tour on the physical device (2026-09-09).** Done and green: sampling one-shot with
projectPath; a long instrumenting session with a build-time weave map through startPaused ->
profile_resume -> profile_snapshot -> profile_pause -> profile_resume -> profile_stop (results hold
only what happened after the tap, as designed); profile_archive/archives and reading an archive back
through sessionId; every read tool (hotspots, flat, tree, callers, callees, threads, timings,
annotate_source); rename/delete; the whole GUI surface (gui_open/view/capture of graph, source,
report, tree, whole window, gui_show/gui_close); and the documented debugger+profiler combination
(sampling attach on the app the debugger runs, debug session intact afterwards).

Five defects the tour found are fixed and committed (12eb8f1..dc8d693): both dsrouter ports checked,
an ambiguous source suffix refused, the session name shown in the listing, duplicate heap types
