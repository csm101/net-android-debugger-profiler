...
| Core/Gui/GuiChannel | the GUI as a driven process (`NapGui.exe --control`): open a session, choose a panel, focus a method, and a rendered PNG back, with no window on screen |
| Mcp/ | stdio MCP server over ProfilerSession; `build_app` prepares an app through Core's `AppBuilder` with a one-word purpose (`BuildPurpose`); `gui_*` shows results as pictures over `GuiChannel` (owned by `GuiHost`) |
