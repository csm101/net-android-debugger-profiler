...
| `NetAndroid.Device/AdbLocator.cs` (shared library since 2026-09-06) | Finds adb without assuming PATH: explicit path (call or `adbPath` in launch.json), `NAD_ADB_PATH`, `ANDROID_HOME` / `ANDROID_SDK_ROOT`, the SDK directory in the registry (`HKCU\SOFTWARE\Novell\Mono for Android\AndroidSdkDirectory`, written by the .NET Android workload; Android Studio's key), the SDK default folders, then PATH. A named source that is wrong is an error, never a fallback. `AdbClient` with no path resolves through it and logs the result |
| `NetAndroid.Device/AdbClient.cs` (shared library; the union of both products' clients) | adb wrapper; every device call takes an explicit serial; `StreamLogcatAsync`, started at the buffer's own boundary (`ReadLogcatBoundaryAsync`: newest stamp + 1 ms, because `logcat -c` does not clear on Android 11); `RunDeviceBytesAsync` for binary `exec-out` |
...
| `NetAndroid.Device/DeviceControl.cs` (shared library) | Drives the screen through adb alone: `screencap` via `exec-out` (binary stdout, `AdbClient.RunDeviceBytesAsync`), `uiautomator dump` to a temp file on the device, `input tap/swipe/keyevent/text`, `wm size/density`, a side-effect-free injection probe (`input keyevent 0`), and `ProbeCapabilitiesAsync`. Vendor refusals to inject surface as `InputInjectionDeniedException` naming the MIUI switch. Independent of the debugger: an agent uses it to reach the point worth debugging and to see the screen at a stop |
| `NetAndroid.Device/UiHierarchy.cs` (shared library) | `UiNode`/`UiTree` records parsed from the uiautomator XML (noise before the XML is skipped - MIUI prints a stack trace first), `UiSelector` matching by text, short or full resource id, short or full class, description, package, flags |
...
| `Mcp/` | `ModelContextProtocol` 2.2.0 stdio server; `DebuggerTools` (one tool = one or two facade calls), `DeviceTools` (screen tools over `DeviceControl`; the device defaults to the session's; `capture_screenshot` returns a `CallToolResult` with an `ImageContentBlock`, the one tool that is not plain text), `SessionHost` (single active session, pid/thread defaults from the last stop), `TextFormat` (plain-text rendering). The DAP frontend has no counterpart for the screen tools: DAP has no such requests |
