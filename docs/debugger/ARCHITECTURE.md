...
| `Core/Device/DeviceControl.cs` | Drives the screen through adb alone: `screencap` via `exec-out` (binary stdout, `AdbClient.RunDeviceBytesAsync`), `uiautomator dump` to a temp file on the device, `input tap/swipe/keyevent/text`, `wm size/density`, a side-effect-free injection probe (`input keyevent 0`), and `ProbeCapabilitiesAsync`. Vendor refusals to inject surface as `InputInjectionDeniedException` naming the MIUI switch. Independent of the debugger: an agent uses it to reach the point worth debugging and to see the screen at a stop |
| `Core/Device/UiHierarchy.cs` | `UiNode`/`UiTree` records parsed from the uiautomator XML (noise before the XML is skipped - MIUI prints a stack trace first), `UiSelector` matching by text, short or full resource id, short or full class, description, package, flags |
