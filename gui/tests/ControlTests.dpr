...
begin
  ...
  LProjects := AClient.Projects(TPath.Combine(ARepoRoot, 'TestTarget\Profiler'), 'Debug');
  Check(Length(LProjects) = 1, Format('%d application project(s) under TestTarget\Profiler', [Length(LProjects)]));
